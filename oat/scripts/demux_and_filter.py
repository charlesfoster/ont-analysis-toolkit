...
def demultiplex_reads(variable_dict):
    ...
    while decision not in [1, 2, 3]:
        ...
        if decision == 1:
            ...
            try:
                cmd = "{0} --input_path {1} --save_path {2} --detect_mid_strand_barcodes --trim_barcodes -x auto --barcode_kits {3}".format(
                ...
                cmd = "{0} --input_path {1} --save_path {2} --detect_mid_strand_barcodes --trim_barcodes --barcode_kits {3}".format(
                    guppy_barcoder, basecalledPath, bcodeDir, barcode_kit_name
                )
                my_log.debug(cmd)
                subprocess.Popen(
